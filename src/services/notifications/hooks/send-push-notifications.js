...
module.exports = function (options = {}) {
  ...
  return async (context) => {
    ...
    if (records.type !== "notification") return context;

    const firebase = context.app.get("firebase");
    switch (records.typeNotification) {
      ...
      case "buyShopUser":
        var params = {
          Message: `Tu pedido en ${records.shop_name} se ha realizado correctamente ID ${records.id}, la tienda aceptará tu pedido en menos de 5 minutos.`,
          PhoneNumber: `${57}${records.phone}`,
          MessageAttributes: {
            "AWS.SNS.SMS.SenderID": {
              DataType: "String",
      ...
      case "orderSend":
        var params = {
          Message: `La tienda ${records.shop.name} ha enviado tu pedido y va camino hacia tí ID ${records.id} `,
          PhoneNumber: `${57}${records.phone}`,
          MessageAttributes: {
            "AWS.SNS.SMS.SenderID": {
              DataType: "String",
